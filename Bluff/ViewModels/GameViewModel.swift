...
class NewGameViewModel: ObservableObject {
    ...
    var isPlayerTurn: Bool {
        return gameData.currentPlayer == .player1
    }
    ...
    private func setupSocket() {
        manager = SocketManager(socketURL: URL(string: "http://<your-server-ip>:5000")!, config: [.log(true), .compress])
        ...
        socket.on("state_update") { [weak self] data, ack in
            guard let self = self else { return }
            if let dict = data.first as? [String: Any] {
                do {
                    let json = try JSONSerialization.data(withJSONObject: dict)
//                    let decoded = try JSONDecoder().decode(GameData.self, from: json)
...
//                    }
    ...
    func updateCurrentStash(_ toUpdate: [CardDetail]) {
        gameData.currentStash[gameData.currentPlayer] = toUpdate
        // Send updated stash to the server
    }
    
    func checkBluff() {
        guard let lastPlayer = gameData.allPlayers.last else { return }
        if let stash = gameData.currentStash[lastPlayer], let firstCard = stash.first {
    ...
    func pass() {
        if let currentIndex = gameData.allPlayers.firstIndex(of: gameData.currentPlayer) {
            ...
            // Notify server about the turn change
        }
    }
